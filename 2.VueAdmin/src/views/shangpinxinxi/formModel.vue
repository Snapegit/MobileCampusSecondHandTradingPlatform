		...
		<el-dialog v-model="formVisible" :title="formTitle" width="80%" destroy-on-close :fullscreen='false'>
			<el-form class="formModel_form" ref="formRef" :model="form" label-width="$template2.back.add.form.base.labelWidth" :rules="rules">
				<el-row>
					...
					<el-col :span="24">
						<el-form-item label="商品标题" prop="shangpinbiaoti">
							<el-input class="list_inp" v-model="form.shangpinbiaoti" placeholder="商品标题"
								 type="text" 								:readonly="!isAdd||disabledForm.shangpinbiaoti?true:false" />
						</el-form-item>
					</el-col>

					<el-col :span="24">
						<el-form-item label="商品成色" prop="shangpinchengse">
							<el-select
								class="list_sel"
								:disabled="!isAdd||disabledForm.shangpinchengse?true:false"
								v-model="form.shangpinchengse" 
								placeholder="请选择商品成色"
								>
								<el-option v-for="(item,index) in shangpinchengseLists" :label="item"
									:value="item"
									>
								</el-option>
							</el-select>
...
<script setup>
	...
	const type = ref('')
	//商品成色列表
	const shangpinchengseLists = ref([])
	//商品类型列表
	...
	const getInfo = ()=>{
		context?.$http({
			...
			res.data.data.shangpinxiangqing = res.data.data.shangpinxiangqing?(res.data.data.shangpinxiangqing.replace(reg,'../../../cl27359140/file')):'';
	...
	const init=(formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null)=>{
		...
		if(formType == 'add'){
			...
			for(let x in row){
				...
				if(x=='shangpinbiaoti'){
					form.value.shangpinbiaoti = row[x];
					disabledForm.value.shangpinbiaoti = true;
					continue;
				}
				if(x=='shangpinchengse'){
					form.value.shangpinchengse = row[x];
					disabledForm.value.shangpinchengse = true;
					continue;
				}
				if(x=='pinpai'){
		...
		shangpinchengseLists.value = "全新,轻微使用痕迹,明显使用痕迹".split(',')
		context?.$http({
